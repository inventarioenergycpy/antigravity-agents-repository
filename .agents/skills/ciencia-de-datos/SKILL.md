...
Para cualquier tarea, consulta o desarrollo de ciencia de datos, SQL, QVDs o reglas BI de EPEC, las siguientes carpetas internas constituyen la **ÚNICA FUENTE DE LA VERDAD**:
- **Esquema Oracle `XXSIGEC` & Text-to-SQL**: [`docs/PROTELEM/PROJECTS/sigec-explorer/`](file:///D:/Proyectos/antigravity-agents-repository/docs/PROTELEM/PROJECTS/sigec-explorer/)
...
> En caso de surgir dudas sobre cuál proyecto de `docs/PROTELEM/PROJECTS/` aplica a una tarea analítica concreta, el agente **DEBERÁ PREGUNTAR AL USUARIO** listando las opciones de fuentes disponibles:
...
5. **Ingeniería Inversa, Diagnóstico y Desarrollo de Proyectos Power BI (`.pbip` / TMDL)**:
   - **Desensamblado TMDL**: Inspección de `.Dataset\definition\tables\*.tmdl` o `.SemanticModel\definition\tables\*.tmdl`, `relationships.tmdl`, `expressions.tmdl` y `model.tmdl`.
   - **Protocolo de Diagnóstico y Validación de Manifiestos PBIP**:
     1. *Strict JSON Schema Keys*: Asegurar que la clave de esquema en `.pbip`, `.pbir` y `.pbi-dataset` sea exactamente `"$schema"` (evitando cadenas vacías `""`).
     2. *Esquemas Canónicos & Coexistencia Dual de Manifiestos (Power BI 2026)*:
